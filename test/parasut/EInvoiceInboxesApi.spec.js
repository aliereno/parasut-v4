...
(function(root, factory) {
  ...
  beforeEach(function() {
    instance = new parasut.EInvoiceInboxesApi();
  });

  describe('(package)', function() {
    describe('EInvoiceInboxesApi', function() {
      describe('listEInvoiceInboxes', function() {
        it('should call listEInvoiceInboxes successfully', function(done) {
          // TODO: uncomment, update parameter values for listEInvoiceInboxes call and complete the assertions
          /*
          var companyId = 56;
          var opts = {};
          opts.filterVkn = 56;
          ...
          instance.listEInvoiceInboxes(companyId, opts, function(error, data, response) {
            if (error) {
              done(error);
              return;
            }
            // TODO: update response assertions
            expect(data).to.be.a(parasut.InlineResponse2006);
            {
              ...
              for (let p in dataCtr) {
                let data = dataCtr[p];
                expect(data).to.be.a(parasut.EInvoiceInbox);
                expect(data.id).to.be.a('string');
                expect(data.id).to.be("");
                expect(data.type).to.be.a('string');
                expect(data.type).to.be("e_invoice_inboxes");
                expect(data.attributes).to.be.a(parasut.EInvoiceInboxAttributes);
                      expect(data.attributes.vkn).to.be.a('string');
                  expect(data.attributes.vkn).to.be("");
                  expect(data.attributes.eInvoiceAddress).to.be.a('string');
                  expect(data.attributes.eInvoiceAddress).to.be("");
                  expect(data.attributes.name).to.be.a('string');
                  expect(data.attributes.name).to.be("");
                  expect(data.attributes.inboxType).to.be.a('string');
                  expect(data.attributes.inboxType).to.be("");
                  expect(data.attributes.addressRegisteredAt).to.be.a(Date);
                  expect(data.attributes.addressRegisteredAt).to.be(new Date());
                  expect(data.attributes.registeredAt).to.be.a(Date);
                  expect(data.attributes.registeredAt).to.be(new Date());
