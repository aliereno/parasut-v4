...
(function(root, factory) {
  ...
  beforeEach(function() {
    instance = new parasut.ApiHomeApi();
  });

  describe('(package)', function() {
    describe('ApiHomeApi', function() {
      describe('showMe', function() {
        it('should call showMe successfully', function(done) {
          // TODO: uncomment, update parameter values for showMe call and complete the assertions
          /*
          var opts = {};
          opts.include = "include_example";

          instance.showMe(opts, function(error, data, response) {
            if (error) {
              done(error);
              return;
            }
            // TODO: update response assertions
            expect(data).to.be.a(parasut.InlineResponse2002);
            expect(data.data).to.be.a(parasut.Me);
                  expect(data.data.id).to.be.a('string');
              expect(data.data.id).to.be("");
              expect(data.data.type).to.be.a('string');
              expect(data.data.type).to.be("users");
              expect(data.data.attributes).to.be.a(parasut.MeAttributes);
                ...
                expect(data.data.attributes.isConfirmed).to.be.a('boolean');
                expect(data.data.attributes.isConfirmed).to.be(false);
              expect(data.data.relationships).to.be.a(parasut.MeRelationships);
                    expect(data.data.relationships.userRoles).to.be.a(parasut.MeRelationshipsUserRoles);
                      {
                    let dataCtr = data.data.relationships.userRoles.data;
                    expect(dataCtr).to.be.an(Array);
                    expect(dataCtr).to.not.be.empty();
                    for (let p in dataCtr) {
                      let data = dataCtr[p];
                      expect(data).to.be.a(parasut.EmployeeRelationshipsManagedByUserRoleData);
                      expect(data.id).to.be.a('string');
                      expect(data.id).to.be("");
                      expect(data.type).to.be.a('string');
                      expect(data.type).to.be("user_roles");
      
                            }
                  }
                expect(data.data.relationships.companies).to.be.a(parasut.MeRelationshipsCompanies);
                      {
                    let dataCtr = data.data.relationships.companies.data;
                    expect(dataCtr).to.be.an(Array);
                    expect(dataCtr).to.not.be.empty();
                    for (let p in dataCtr) {
                      let data = dataCtr[p];
                      expect(data).to.be.a(parasut.AddressRelationshipsAddressableData);
                      expect(data.id).to.be.a('string');
                      expect(data.id).to.be("");
                      expect(data.type).to.be.a('string');
                      expect(data.type).to.be("companies");
      
                            }
                  }
                expect(data.data.relationships.profile).to.be.a(parasut.MeRelationshipsProfile);
                      expect(data.data.relationships.profile.data).to.be.a(parasut.MeRelationshipsProfileData);
                        expect(data.data.relationships.profile.data.id).to.be.a('string');
                    expect(data.data.relationships.profile.data.id).to.be("");
                    expect(data.data.relationships.profile.data.type).to.be.a('string');
                    expect(data.data.relationships.profile.data.type).to.be("profiles");
            {
              ...
              for (let p in dataCtr) {
                let data = dataCtr[p];
                expect(data).to.be.a(parasut.InlineResponse2002Included);
                expect(data.id).to.be.a('string');
                expect(data.id).to.be("");
                expect(data.type).to.be.a('string');
                expect(data.type).to.be("user_roles");
